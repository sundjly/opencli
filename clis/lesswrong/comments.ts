...
cli({
  ...
  func: async (_page, kwargs) => {
    ...
    const comments = (commentsData?.comments?.results ?? []) as Array<Record<string, unknown>>;
    ...
    for (let i = 0; i < comments.length; i++) {
      ...
      const user = item.user as { displayName?: string } | undefined;
      const raw = stripHtml((item.htmlBody as string) ?? '');
      rows.push({
        rank: i + 1,
        score: (item.baseScore as number) ?? 0,
