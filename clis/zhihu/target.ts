...
export type ZhihuTarget =
  ...
  | { kind: 'answer'; questionId: string; id: string; url: string }
...
function isAllowedZhihuUrl(url: URL): boolean {
  return url.protocol === 'https:' && url.username === '' && url.password === '' && url.port === '';
}

export function parseTarget(input: string): ZhihuTarget {
  const value = String(input).trim();
  if (EMPTY_AUTHORITY_RE.test(value)) {
    throw new CliError(
      'INVALID_INPUT',
      'Zhihu write commands require a normal HTTPS Zhihu URL without malformed authority',
  ...
  if (value.startsWith('answer:') && !ANSWER_RE.test(value)) {
    throw new CliError(
      'INVALID_INPUT',
      'Invalid answer target, expected answer:<questionId>:<answerId>',
      'Example: opencli zhihu like answer:123:456 --execute',
    );
  }
  ...
  throw new CliError(
    'INVALID_INPUT',
    'Zhihu write commands require a Zhihu URL or typed target like question:123 or answer:123:456',
    'Example: opencli zhihu like answer:123:456 --execute',
  );
}

export function assertAllowedKinds(command: string, target: ZhihuTarget): ZhihuTarget {
  const allowed: Record<string, ZhihuTarget['kind'][]> = {
