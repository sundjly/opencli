...
cli({
  ...
  func: async (page: IPage | null, kwargs: Record<string, unknown>) => {
    if (!page) throw new CommandExecutionError('Browser session required for zhihu favorite');
    ...
    const preflight = await page.evaluate(`(async () => {
      ...
      wrongAnswer?: boolean;
      missingChooser?: boolean;
      chooserRows: FavoriteRow[];
    ...
    const matchingRows = preflight.chooserRows.filter((row) => (
      collectionId
        ? row.id === collectionId
    ...
    if (!selectedBefore.every((row) => verify.selectedAfter.includes(row))) {
      throw new CliError(
        'OUTCOME_UNKNOWN',
        `Favorite action changed unrelated collection membership: before=${JSON.stringify(selectedBefore)} after=${JSON.stringify(verify.selectedAfter)}`,
      );
    }

    const outcome = targetRow.selected ? 'already_applied' : 'applied';
    return buildResultRow(
      targetRow.selected ? `Already favorited ${target.kind}` : `Favorited ${target.kind}`,
      target.kind,
      rawTarget,
      outcome,
      {
