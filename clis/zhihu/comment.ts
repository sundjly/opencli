...
cli({
  ...
  func: async (page: IPage | null, kwargs: Record<string, unknown>) => {
    if (!page) throw new CommandExecutionError('Browser session required for zhihu comment');
    ...
    })()`) as { entryPathSafe?: boolean; wrongAnswer?: boolean };
    ...
    })()`) as { editorContent?: string; mode?: string };
    ...
    const proof = await page.evaluate(`(async () => {
      ...
      proofType?: string;
      createdProof?: Record<string, unknown>;
      createdUrl?: string;
      commentScope?: string;
