...
describe('zhihu favorite', () => {
  ...
  it('requires persisted read-back and preserves previously selected collections', async () => {
    ...
    await expect(
      cmd!.func!(page, { target: 'article:1', collection: '默认收藏夹', execute: true }),
    ).resolves.toEqual([
  ...
  it('accepts --collection-id as the stable selector path', async () => {
    ...
    await expect(
      cmd!.func!(page, { target: 'article:1', 'collection-id': 'fav-b', execute: true }),
    ).resolves.toEqual([
  ...
  it('reuses data-attribute answer anchoring during reopened chooser verification', async () => {
    ...
    await expect(
      cmd!.func!(page, { target: 'answer:1:2', 'collection-id': 'fav-b', execute: true }),
    ).resolves.toEqual([
