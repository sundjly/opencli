...
function apiGiteeIndex(user: Record<string, unknown> | null): string {
  if (!user) return '';
  const keys = [
    'gitee_index',
    'giteeIndex',
...
cli({
  ...
  func: async (page, args) => {
    ...
    const domSnapshot: DomUserSnapshot = {
    ...
    if (domSnapshot.notFound) {
      throw new CliError(
        ...
        'Check the username and retry: opencli gitee user <username>',
      );
    }
    ...
    if (domSnapshot.blocked) {
      throw new CliError(
        ...
        'The profile may be private/restricted, or the account may be unavailable',
      );
    }
    ...
    if (!apiResponse.ok) {
      throw new CliError(
        ...
        'Try again later or verify network access to gitee.com',
      );
    }

    const apiUser = asRecord(await apiResponse.json() as unknown);
    const nickname = pickFirst(
      domSnapshot.nickname,
      firstText(apiUser?.name),
      firstText(apiUser?.login),
      username,
    );
    const followers = pickFirst(
      domSnapshot.followers,
      normalizeCount(apiUser?.followers),
      '-',
    );
    const publicRepos = pickFirst(
      domSnapshot.publicRepos,
      normalizeCount(apiUser?.public_repos),
      '-',
    );
