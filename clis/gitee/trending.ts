...
function normalizeStars(value: string): string {
...
function normalizeRepoUrl(value: string): string | null {
  try {
    ...
    if (!GITEE_HOSTS.has(parsed.hostname.toLowerCase())) return null;
    ...
    const owner = parts[0];
    const repo = parts[1];
    if (RESERVED_SEGMENTS.has(owner.toLowerCase()) || RESERVED_SEGMENTS.has(repo.toLowerCase())) return null;
...
function repoUrlFromPath(value: string): string | null {
  const compact = value.trim().replace(/^\/+|\/+$/g, '');
  if (!compact.includes('/')) return null;
  return normalizeRepoUrl(`https://gitee.com/${compact}`);
}

function toCaptureProject(record: Record<string, unknown>): CaptureProject | null {
  const urlCandidates: string[] = [];
  const pushUrlCandidate = (raw: unknown): void => {
    if (typeof raw !== 'string') return;
    const normalized = normalizeRepoUrl(raw) ?? repoUrlFromPath(raw);
    if (normalized) urlCandidates.push(normalized);
  };
...
function parseCaptureBody(entry: unknown): unknown | null {
  if (!isRecord(entry)) return null;
  const preview = typeof entry.responsePreview === 'string' ? entry.responsePreview : '';
  if (!preview || preview.startsWith('base64:')) return null;
...
function choosePreferredText(current: string | undefined, incoming: string | undefined): string | undefined {
  if (!incoming || incoming === '-') return current;
  if (!current || current === '-') return incoming;
  return incoming.length > current.length ? incoming : current;
}

function collectProjectsFromUnknown(
  value: unknown,
  out: Map<string, CaptureProject>,
  seen: Set<unknown>,
  depth: number,
): void {
...
function collectProjectsFromCapture(entries: unknown[]): Map<string, CaptureProject> {
  const collected = new Map<string, CaptureProject>();
...
function toProject(value: unknown): GiteeProject | null {
...
function mergeCapturedProject(project: GiteeProject, captured: CaptureProject | undefined): GiteeProject {
  if (!captured) return project;
...
cli({
  ...
  func: async (page, args) => {
    ...
    let projectsFromCapture = new Map<string, CaptureProject>();
    ...
    if (!Array.isArray(rawProjects)) {
      throw new CliError(
        ...
        'Gitee may have changed its page structure',
      );
    }
    ...
    if (projects.length === 0) {
      throw new CliError(
        ...
        'Gitee may be blocking this request or the page structure changed',
      );
    }
